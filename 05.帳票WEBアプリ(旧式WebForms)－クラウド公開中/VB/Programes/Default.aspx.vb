...
Partial Public Class _Default
    ...
    Protected Sub Page_Load(sender As Object, e As EventArgs)
        ' PDFストリーム要求の場合
        If Request.QueryString("action") = "pdf" Then
            GeneratePdfForView()
            Return
        End If
    End Sub
    ...
    Protected Sub btnOutputPDF_Click(sender As Object, e As EventArgs)
        Dim b As Button = DirectCast(sender, Button)
        If b.ID = "btnOutputPDF" Then
...
"<html>" & vbCrLf &
"<head>" & vbCrLf &
"    <title>PDF Viewer</title>" & vbCrLf &
"    <style>" & vbCrLf &
"        body, html { margin: 0; padding: 0; height: 100%; overflow: hidden; }" & vbCrLf &
"        iframe { border: none; width: 100%; height: 100%; }" & vbCrLf &
"    </style>" & vbCrLf &
"</head>" & vbCrLf &
"<body>" & vbCrLf &
"    <iframe src=""Default.aspx?action=pdf""></iframe>" & vbCrLf &
"</body>" & vbCrLf &
"</html>"
    ...
    Private Function GetSelectedSampleType() As String
        If radSampleSimple.Checked Then Return "simple"
        If radSample10の倍数.Checked Then Return "simple10"
        If radSample見積書.Checked Then Return "mitsumori"
        If radSample郵便番号.Checked Then Return "zipcode"
        If radSample広告.Checked Then Return "koukoku"
        If radSample請求書.Checked Then Return "invoice"
        If radSample商品一覧.Checked Then Return "itemlist"
        Return "simple"
    End Function

    Private Sub GeneratePdfForView()
        Dim sampleType As String = If(Session("SelectedSample")?.ToString(), "simple")
    ...
    Private Sub GeneratePdfForDownload()
        ' 現在、Pao@Office 保有のAzure上のSQL Server に接続する設定にしてあります。
        sqlcon = New SqlConnection("Server=tcp:fzxu46e9ck.database.windows.net,1433;Initial Catalog=Reports.net.Sample;Persist Security Info=False;User ID=login_user;Password=$Abc123$;MultipleActiveResultSets=False;Encrypt=True;TrustServerCertificate=False;Connection Timeout=30;")

        ' PDF出力インスタンスの生成
        Dim paoRep As IReport = ReportCreator.GetPdf()
        If radSampleSimple.Checked Then
            makeReports単純なサンプル(paoRep)
        ElseIf radSample10の倍数.Checked Then
            makeReports10の倍数(paoRep)
        ElseIf radSample見積書.Checked Then
            paoRep = ReportCreator.GetImagePdf()
            makeReports見積書(paoRep)
        ElseIf radSample郵便番号.Checked Then
            makeReports郵便番号(paoRep)
        ElseIf radSample広告.Checked Then
            paoRep = ReportCreator.GetImagePdf()
            makeReports広告(paoRep)
        ElseIf radSample請求書.Checked Then
            makeReports請求書(paoRep)
        ElseIf radSample商品一覧.Checked Then
            makeReports商品一覧(paoRep)
        Else
            Return
        End If
        ...
        Dim pdfFilePath As String = System.IO.Path.Combine(HttpRuntime.AppDomainAppPath, "App_Data\DownloadPDF.pdf")
    ...
    Protected Sub makeReports単純なサンプル(paoRep As IReport)
        ...
        paoRep.Write("Text2", "これはAzure ASP.NETで作った" & vbLf & "印刷データですよ～ん♪")
        paoRep.PageEnd()
    ...
    Protected Sub makeReports郵便番号(paoRep As IReport)
    ...
    Protected Sub makeReports広告(paoRep As IReport)
        ' デザインファイル
    ...
    Protected Sub makeReports請求書(paoRep As IReport)
        ' デザインファイル
    ...
    End Class
    Protected Sub makeReports商品一覧(paoRep As IReport)
        ' デザインファイル
